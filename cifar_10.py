...
rects1 = plt.bar(index, acc_resnet, bar_width,
alpha=opacity,
color='b',
label='ResNet')

rects2 = plt.bar(index + bar_width, acc_densenet, bar_width,
alpha=opacity,
color='g',
label='DenseNet')

rects3 = plt.bar(index + 2*bar_width, acc_vgg, bar_width,
alpha=opacity,
color='r',
label='VGG')

rects3 = plt.bar(index + 3*bar_width, acc_resnext, bar_width,
alpha=opacity,
color='y',
label='ResNext')
...
# model.fc=Network(input_size_last_layer,len(classes),hidden_layers) # has to be adapted according to model
#
# use_pretrained_model(model,classes,learning_rate, regularization_weight,train_dataset, test_dataset,batch_size,device,num_epochs,print_every)
#
